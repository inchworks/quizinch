...
Appliance operation requires some packages that are not pre-installed on Raspberry Pi OS: `sudo apt update` and `sudo apt install dialog unclutter`.

Because the appliance files change the configuration of the RPi, you must copy them to the appropriate locations yourself, by running a script supplied by QuizInch: `sh /srv/quizinch/setup/appliance.sh`. Or if you prefer, use the supplied files as an example for your own setup.
...
- To set your own network password, edit this file to change `wifi-sec.psk` from "quizinch.ap". Alternatively, change the password using the appliance menu after setup.
- Optionally, change `ssid` from QUIZ-RPI to your own.
- One way to edit the file is `sudo nano /srv/quizinch/setup/appliance.sh`.
